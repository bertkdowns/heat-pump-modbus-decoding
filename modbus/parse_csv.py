...
class PacketParseError(Exception):
    """Custom exception for packet parsing errors."""
    pass

class PacketSkip(PacketParseError):
    """Exception to indicate a packet should be skipped. This is for e.g a 8 byte read request packet."""
    pass

class PacketParser:
    last_request_slave_id = None
    last_request_function = None
    last_request_address = 0

    def parse_packet(self, packet_bytes: bytes) -> tuple[int, dict[int,int]]:
        """
        Docstring for parse_packet
        
        :param packet_bytes: Description
        :type packet_bytes: bytes
        :return: a tuple of (slave_id, registers) where `registers` is a dictionary mapping register_address to register_value
        """
        if len(packet_bytes) < 8:
            raise PacketParseError(f"Packet too short")
        ...
        if received_crc != calculated_crc:
            raise PacketParseError(f"CRC mismatch: received {received_crc:04X}, calculated {calculated_crc:04X}")
        ...
        if len(packet_bytes) == 8:
            ...
            self.last_request_slave_id = (int) (packet_bytes[0])
            self.last_request_function = (int) (packet_bytes[1])
            self.last_request_address = int.from_bytes(packet_bytes[2:4], byteorder='big')
            raise PacketSkip("Request packet, no data to parse")
        ...
        if function == 3: # read holding registers
            ...
            if slave_id != self.last_request_slave_id or function != self.last_request_function:
                raise PacketParseError(f"Slave ID or function mismatch")
            ...
            if num_registers != available_values:
                raise PacketParseError(f"Number of registers mismatch: expected {num_registers}, got {available_values}")
            
            registers = {}
            for i in range(num_registers):
                register_address = self.last_request_address + i
                register_value = int.from_bytes(packet_bytes[3 + i*2: 5 + i*2], byteorder='big')
                #raise PacketParseError(f"Data at index {index}: address={register_address}, value={register_value}")
            ...
            if num_of_registers != available_values:
                raise PacketParseError(f"Number of registers mismatch!: expected {num_of_registers}, got {available_values}")
            ...
            raise PacketParseError(f"Unknown function {function}")
...
def parse_csv(file_path: str) -> pd.DataFrame:
    ...
    with open(file_path, mode='r', newline='') as csvfile:
        ...
        for row in reader:
            ...
            try:
                slave_id, registers = parser.parse_packet(packet_bytes)
            except PacketSkip:
                # Just skip this packet, iut only updates the last request info
                continue
            except PacketParseError as e:
                print(f"Error parsing packet at index {index}: {e}")
                continue

            for register_address, register_value in registers.items():
